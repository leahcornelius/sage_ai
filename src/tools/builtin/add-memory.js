...
function createAddMemoryHandler({ config, memoryService }) {
  return async function handleAddMemory({ args, logger }) {
    ...
    const whitelist = new Set(
      config.tools.memoryWriteWhitelist || [addMemoryTool.function.name]
    );
    if (!whitelist.has(addMemoryTool.function.name)) {
      throw new AppError({
        statusCode: 403,
        code: "memory_write_not_allowed",
        type: "invalid_request_error",
        message: `Tool ${addMemoryTool.function.name} is not allowed to write memory.`,
      });
    }
    ...
    const result = await memoryService.addMemoryFromTool({
      ...
      category,
      eventTime,
      toolName: addMemoryTool.function.name,
      logger,
    });
