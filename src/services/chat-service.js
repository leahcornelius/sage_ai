...
function createChatService({
  ...
  const modelRouter = llmRouter || createDirectRouter(openaiClient);
  ...
  async function createChatCompletion({
    ...
    const upstreamRequest = buildUpstreamRequest({
      requestBody,
      promptService,
      memoryContext: memoryContextResult.contextBlock,
      logger: operationLogger,
    });
    ...
    const completion = shouldExecuteTools({ requestBody, toolsEnabled: executionContext.tools.length > 0 })
      ...
      : await modelRouter.createChatCompletion(upstreamRequest, {
        signal,
        logger: operationLogger,
        requestId,
      });
    operationLogger.info(
      {
        ...
        memoryCacheHit: memoryContextResult.cacheHit,
        memoryPartial: memoryContextResult.partial,
      },
  ...
  async function* streamChatCompletion({ requestBody, signal, logger: requestLogger, requestId }) {
    const operationLogger = requestLogger || serviceLogger;
    await modelService.assertModelAvailable(requestBody.model, { logger: operationLogger });
    await synchronizeConversationForRequest({
      conversationStore,
    ...
    try {
      if (shouldExecuteTools({ requestBody, toolsEnabled: executionContext.tools.length > 0 })) {
        ...
        const stream = await modelRouter.streamChatCompletion(upstreamRequest, {
          signal,
          logger: operationLogger,
          requestId,
        });
...
async function* runStreamingToolLoop({
  ...
  for (let round = 0; round < maxRounds; round += 1) {
    const stream = await llmRouter.streamChatCompletion(
      {
        ...basePayload,
        messages: conversationMessages,
      },
...
function buildUpstreamRequest({ requestBody, promptService, memoryContext, logger }) {
  ...
  logger?.debug(
    {
      ...
      memoryContextLength: textLength(resolvedMemoryContext),
    },
...
function scheduleConversationMemoryIngestion({
  memoryService,
  conversationId,
  assistantMessage,
  modelId,
  ...
  if (!conversationId || !assistantMessage) {
    logger.debug(
      ...
      "Skipping background memory ingestion due to missing conversation context"
    );
    return;
  }
  logger.debug(
    ...
    "Scheduling background memory ingestion"
  );
  ...
  void memoryService
    ...
    .catch((error) => {
      logger.warn({ err: error }, "Background memory ingestion failed");
    });
}
...
function deriveLastUserAssistantTurnIndex(messages) {
  ...
  return Math.max(0, uaCount - 1);
}

function createDirectRouter(openaiClient) {
  return {
    async createChatCompletion(payload, { signal }) {
      return openaiClient.chat.completions.create(payload, { signal });
    },
