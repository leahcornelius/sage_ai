...
```json
{
  "status": "ok",
  "memory": {
    "mem0": { "status": "ok|disabled|error" },
    "zep": { "status": "ok|disabled|error" },
    "redis": { "status": "ok|disabled|error" },
    "mnemosyne": { "status": "ok|disabled|error" }
  }
}
```
...
- If request `model` is absent and `SAGE_DEFAULT_MODEL` is set, Sage uses the default model.
- If `SAGE_ALLOW_MODEL_OVERRIDE=false` and request `model` differs from `SAGE_DEFAULT_MODEL`, Sage returns `400`.
