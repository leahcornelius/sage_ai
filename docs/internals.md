...
| `src/http/routes/health.js` | `registerHealthRoutes` | `/health` endpoint + memory backend statuses | `src/app.js` |
| `src/http/routes/models.js` | `registerModelRoutes` | `/v1/models` endpoint | `src/app.js` |
| `src/http/routes/chat-completions.js` | `registerChatCompletionRoutes` | `/v1/chat/completions`, SSE writing | `src/app.js` |
...
| `src/services/llm-router-service.js` | `createLlmRouterService` | local-first LLM routing and cloud fallback policy | `src/index.js`, chat service |
...
| `src/services/memory-service.js` | `createMemoryService` | Memory facade API backed by memory controller | `src/index.js`, chat/tool layers |
| `src/services/memory/memory-controller.js` | `createMemoryController` | Retrieval budgeting, adapter orchestration, idempotent writes | `memory-service` |
| `src/services/memory/mem0-adapter.js` | `createMem0Adapter` | Write-path structured fact extraction | `memory-controller` |
| `src/services/memory/zep-adapter.js` | `createZepAdapter` | Graph fact upsert and graph query | `memory-controller` |
| `src/services/memory/mnemosyne-adapter.js` | `createMnemosyneAdapter` | Episodic + semantic persistence/search | `memory-controller` |
| `src/services/memory/redis-cache.js` | `createRedisCache` | Identity/query memory caches and invalidation | `memory-controller` |
...
| `SAGE_DEFAULT_MODEL` | unset | Default chat model when request omits `model` |
| `SAGE_ALLOW_MODEL_OVERRIDE` | `true` | Allows/disallows API `model` overriding `SAGE_DEFAULT_MODEL` |
...
| `SAGE_MEMORY_EXTRACTION_ALLOW_MODEL_OVERRIDE` | `false` | Allows/disallows request `model` overriding extraction model |
| `SAGE_MEMORY_SUMMARY_MODEL` | unset | Optional model for rolling summary updates |
| `SAGE_MEMORY_SUMMARY_ALLOW_MODEL_OVERRIDE` | `false` | Allows/disallows request `model` overriding summary model |
