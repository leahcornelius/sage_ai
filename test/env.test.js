...
test("createConfig parses optional settings and defaults", () => {
  ...
  assert.equal(config.memory.mode, "hard");
  assert.equal(config.memory.identityCacheTtlSec, 300);
  assert.equal(config.memory.embeddingProvider, "mnemosyne");
  assert.equal(config.memory.graphMaxResults, 20);
  assert.equal(config.memory.writeConcurrencyLimit, 8);
  ...
  assert.equal(config.memory.extractionAllowModelOverride, false);
  assert.equal(config.memory.summaryModel, null);
  assert.equal(config.memory.summaryAllowModelOverride, false);
  assert.equal(
  ...
  assert.deepEqual(config.tools.memoryWriteWhitelist, ["add_memory"]);
...
test("createConfig parses memory extraction cadence and conversation db path", () => {
  const config = createConfig({
    ...
    SAGE_MEMORY_EXTRACTION_ALLOW_MODEL_OVERRIDE: "true",
    SAGE_MEMORY_SUMMARY_ALLOW_MODEL_OVERRIDE: "true",
  ...
  assert.equal(config.memory.extractionAllowModelOverride, true);
  assert.equal(config.memory.summaryAllowModelOverride, true);
  assert.equal(
    config.memory.conversationDbPath,
...
test("createConfig parses memory runtime mode and tool write whitelist", () => {
  const config = createConfig({
    OPENAI_API_KEY: "openai-key",
    SAGE_API_KEY: "sage-key",
    BRAVE_API_KEY: "brave-key",
    ...
    SAGE_REDIS_ENABLED: "true",
    SAGE_MEMORY_TOOL_WRITE_WHITELIST: "add_memory,admin_memory_write",
  });

  assert.equal(config.memory.mode, "soft");
  assert.equal(config.memory.mem0Enabled, false);
...
test("createConfig parses default model and override policy", () => {
  const config = createConfig({
    OPENAI_API_KEY: "openai-key",
    SAGE_API_KEY: "sage-key",
    BRAVE_API_KEY: "brave-key",
    SAGE_DEFAULT_MODEL: "gpt-5.2-mini",
    SAGE_ALLOW_MODEL_OVERRIDE: "false",
  });

  assert.equal(config.openai.defaultModel, "gpt-5.2-mini");
  assert.equal(config.openai.allowModelOverride, false);
});

test("createConfig parses llm router endpoints and warmup timeout", () => {
  const config = createConfig({
    OPENAI_API_KEY: "openai-key",
    SAGE_API_KEY: "sage-key",
    BRAVE_API_KEY: "brave-key",
    SAGE_LLM_LOCAL_BASE_URL: "http://localhost:11434/v1",
    SAGE_LLM_CLOUD_BASE_URL: "https://api.openai.com/v1",
    SAGE_LLM_LOCAL_STREAM_WARMUP_MS: "2500",
  });

  assert.equal(config.llm.localBaseUrl, "http://localhost:11434/v1");
  assert.equal(config.llm.cloudBaseUrl, "https://api.openai.com/v1");
  assert.equal(config.llm.localStreamWarmupMs, 2500);
});

test("createConfig requires default model when model override is disabled", () => {
  assert.throws(
    () =>
      createConfig({
        OPENAI_API_KEY: "openai-key",
